---
{"aliases":null,"tags":null,"description":"문맥에 따른 비트해석, 컴파일의 단계, 프로세서 해부학, 운영체제와 가상 메모리, 메모리 계층구조에 대해 공부한다.","title":"1. A Tour of Computer Systems","created":"2023-08-30T17:42:17","updated":"2023-08-30T17:46:02","dg-publish":true,"permalink":"/docs/1. A Tour of Computer Systems/","dgPassFrontmatter":true}
---
...
**시스템을 구성하는 하드웨어 컴포넌트**
- Buses  [[docs/buses and hierarchical buses {os}\|buses and hierarchical buses {os}]]
- I/O Devices
- Main Memory
- Processor
...
- [?] ~~왜 스택이 위에서 아래로 자라고 힙이 아래에서 위로 자라지? `brk`는 어디에 붙어있더라? ~~
	- [[docs/04. process {SP}\|04. process {SP}]] 에서 볼 수 있듯이, 스택은 높은 주소에서부터 시작해 아래로 향하고, 힙은 낮은 주소에서부터 시작해 올라간다.  `brk`는 힙의 경계에 붙어있다.
	- [?] Memory-mapped region for shared libraries는 어디에 끼어있는거지?  
![스크린샷 2023-08-27 오후 9.33.41.png|800](/img/user/docs/assets/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202023-08-27%20%EC%98%A4%ED%9B%84%209.33.41.png)
...
- Layer and Abstractions
    ...
    > 소프트웨어 공학의 역사는 추상화 수준을 높히는 방향으로 진행되어왔다. |  [[docs/20230522 김충환 회고강사특강#협력을 통한 추상화\|20230522 김충환 회고강사특강#협력을 통한 추상화]] 
